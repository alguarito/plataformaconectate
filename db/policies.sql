...
--    El acceso de lectura/actualización se hace por id + OTP desde el backend
--    con service_role, no desde el cliente.
--
--    Diseño:
--    - Anon PUEDE INSERTAR (iniciar flujo de registro).
--    - Anon NO PUEDE SELECT, UPDATE ni DELETE (privacidad: no debe enumerar
--      registros de otros). RLS habilitada + ninguna policy de SELECT/UPDATE/
--      DELETE = default deny automático.
-- ============================================================================

-- El cliente anónimo puede crear un registro (al ingresar el código de aula).
-- Roles explícitos: anon (visitante sin sesión) y authenticated (estudiante
-- que ingresa otro estudiante de su misma aula, por ejemplo).

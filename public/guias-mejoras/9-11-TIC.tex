...
2. Reconocimiento de patrones & \textbf{Reconocimiento de patrones.} Toda métrica útil sigue el patrón \textbf{``observa-decide''}: una cifra que no lleva a una decisión es vanidad disfrazada de información. Antes de mirar la métrica, pregúntate: \emph{¿qué haría diferente si esta cifra fuera el doble o la mitad?}. Si la respuesta es ``nada'', no la mires. Es el filtro que separa al estudiante que aprende con datos del que se obsesiona con likes. La diferencia: el aprendiz pregunta ``¿qué haré distinto?''; el adicto al feed pregunta ``¿cuánto subió?''.\\
3. Abstracción & \textbf{Abstracción.} Lo esencial del análisis cabe en una pregunta: \emph{¿esta cifra confirma o desafía mi hipótesis sobre quién visita mi sitio y por qué?}. Si solo confirma, no aprendes nada nuevo (solo te sientes bien). Si desafía, ajustas tu hipótesis --- ahí está el aprendizaje. Las métricas que más enseñan son las que te sorprenden. Si nunca te sorprenden, probablemente no estás midiendo lo que importa.\\
4. Algoritmo conceptual & \textbf{Algoritmo.} Pasos semanales: (1) lunes 7 PM, abrir el dashboard; (2) anotar las 5 cifras de la semana en el cuaderno; (3) compararlas con la semana anterior (no en absoluto, sino en tendencia: \texttt{+}, \texttt{-}, \texttt{=}); (4) identificar una sorpresa (algo que no esperabas); (5) escribir 1 decisión concreta y accionable que tomarás esta semana basada en esos datos; (6) cerrar el dashboard hasta el próximo lunes --- la disciplina semanal vence al refresh diario.\\
\end{tabularx}
\normalsize

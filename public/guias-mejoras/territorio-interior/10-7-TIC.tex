...
\begin{softbox}{milcTurquesa}{milcGris}{Cuatro claves para desarmar un rumor}
...
1. Las tres capas & \textbf{Todo mensaje tiene tres capas.} La \textbf{macroestructura} es lo que afirma en el fondo, resumible en una frase: es lo único que la gente recuerda y repite. La \textbf{microestructura} es cómo encadena las frases, y ahí viven los conectores: «y por eso», «entonces», «así que». Un conector causal \textbf{no prueba} una causa, le ordena al lector que la construya. Y los \textbf{modalizadores} («al parecer», «dicen que», «me confirmaron») son el blindaje: afirman sin que nadie quede afirmando. Bórralos y el mensaje se queda sin ninguna fuente.\\
2. La cuenta & \textbf{La cuenta desarma el rumor mejor que la indignación.} Tomemos el más peligroso del regreso a la normalidad: «échale media taza de cloro al tanque». Media taza son unos 120 mL; un blanqueador comercial al 5 \% aporta 50 mg de cloro por mL. Eso da 6.000 mg, que alcanzarían para tratar unos \textbf{2.400 litros}. Un tanque doméstico corriente tiene 500. Y fíjate en lo decisivo: \textbf{el mensaje nunca dice el volumen del tanque}. Sin volumen no hay concentración, y sin concentración esa instrucción no significa nada. Un dato al que le falta el denominador no es un dato.\\
3. La fuente & \textbf{Toda cifra tiene un origen, y a veces el origen existe.} El rumor del cloro no salió de la nada: nació de mezclar dos dosis reales del mismo documento oficial, una para \textbf{lavar las paredes del tanque} (que se enjuaga después) y otra, quince veces menor, para el agua que se bebe. Rastrear el origen es más útil que desmentir. Y hay una trampa nueva: hoy un buscador puede atribuirle a un ministerio una recomendación que ese ministerio dio \textbf{para otra cosa}. La única defensa es abrir el documento citado y leer para qué era.\\
4. Hecho y sentido & \textbf{Hecho y sentido no se tratan igual.} Un \textbf{reclamo de hecho} («el agua está contaminada») se verifica, venga de quien venga. Un \textbf{reclamo de sentido} («esto fue un llamado de atención», «Dios nos cuidó») pertenece a la tradición de cada quien y no se somete a comprobación: aquí no se discute, se respeta. La frontera se cruza solo cuando un reclamo de sentido \textbf{se convierte en instrucción} («no hay que prepararse porque todo está escrito»), porque ahí ya hay una consecuencia que se puede verificar y de la que alguien responde.\\
\end{tabularx}
\normalsize
...
\guiaDiagrama{assets/10-7/cuenta-del-rumor.tikz}{La cuenta completa, con sus bases a la vista. Esta guía no te dice cuánto cloro echar, y esa omisión es deliberada: para eso está tu autoridad sanitaria, no un cuaderno de clase ni una cadena de WhatsApp.}

\textbf{En tu cuaderno (Actividad 2 · ANALIZA).} Título: «Actividad 2. El mensaje abierto en capas». \textbf{Formato:} toma \textbf{uno} de tus cinco mensajes y ábrelo como en la figura: su macroestructura en una sola frase, los conectores que fabrican causa, y la lista de modalizadores. \textbf{Extensión:} el análisis de las tres capas más la cuenta, si tu mensaje trae alguna cifra. \textbf{Sabes que terminaste cuando} puedes decir qué le queda al mensaje si le borras los modalizadores: casi siempre, nada.
\end{infoband}
...
Verificar & \textbf{Verificar:} escojan tres mensajes de los que recogieron. De cada uno, la fuente con nombre, la fecha de corte del dato y una segunda fuente independiente. Si no consiguen la segunda, el boletín lo dice: «no pudimos confirmarlo». Esa frase vale más que un desmentido apurado.\\
Calcular & \textbf{Calcular:} si el mensaje trae una cifra, háganla. Muestren la cuenta completa, con las unidades y con la base que asumieron. Una cuenta a la vista convence a quien no le cree a la autoridad, porque puede repetirla.\\
Escribir & \textbf{Escribir:} una frase por rumor con la macroestructura corregida, en lenguaje de barrio y sin tecnicismos. Luego el dato con su fuente. Y el cierre que ninguna cadena tiene: quién lo escribió, cuándo y dónde preguntar más.\\
Cuidar & \textbf{Cuidar:} el boletín no es un regaño. Nadie que haya reenviado algo falso lo hizo por malicia: lo hizo por miedo, que es lo más humano que hay. Incluyan un recuadro que lo diga y que recuerde a quién acudir: orientación escolar, un docente, la familia.\\
\end{tabularx}
\normalsize

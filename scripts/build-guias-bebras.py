#!/usr/bin/env python3
"""
Motor de generación de guías PDF para el espacio **Bebras** (Explora).
...
y produce:
    public/guias-mejoras/bebras/momento-{N}-TIC.pdf   (vía xelatex, 2 pasadas)
...
Uso:
    python3 scripts/build-guias-bebras.py 0        # solo el Momento 0
    python3 scripts/build-guias-bebras.py 0 1 2    # varios
    python3 scripts/build-guias-bebras.py          # todos los completos
"""
...
# primario en la portada), reusando la lima ya probada del grado 7.
COLORES = {
...
# de grados, pero Bebras es transversal: un solo bloque para todas las guías).
META = {
    "PERIODO_NOMBRE": "Curso «Pensar como una máquina» · Pensamiento computacional",
...
def cargar_guia(momento: str) -> dict | None:
    """Lee una guía Bebras por número de momento (ej. '0' → momento-0.yaml)."""
    path = CONTENT_DIR / f"momento-{momento}.yaml"
    if not path.exists():
        return None
...
def yaml_a_placeholders(guia: dict) -> dict[str, str]:
    ...
    y el del header (GUIA_NUMERO) toman el número de MOMENTO (Bebras no usa
...
def main(argv: list[str]) -> int:
    ...
    if argv:
        ...
        seleccion = sorted(
            p.stem.replace("momento-", "") for p in CONTENT_DIR.glob("momento-*.yaml")
        )

    completas, errores = [], []
    for momento in seleccion:
        guia = cargar_guia(momento)
        if guia is None:
            print(f"  ?  momento-{momento} — no existe en {CONTENT_DIR}")

...
def is_octavo_data_phronesis(guide: GuideData) -> bool:
    return guide.grade == 8 and guide.number == 1
...
def make_question_for_guide(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "¿Cómo convertir saberes del entorno y datos actuales en una decisión prudente, clara y responsable?"
    if family == "systems_room":
...
def make_product_for_guide(guide: GuideData) -> str:
    family = guide_family(guide)
    if is_octavo_data_phronesis(guide):
        return "Mini análisis de datos con pregunta, tabla limpia, gráfica simple, conclusión prudente y reflexión ciudadana."
...
def concept_rows(guide: GuideData) -> list[tuple[str, str, str]]:
    ...
    if is_octavo_data_phronesis(guide):
        return [
            ("Dato", "Registro de una característica observada o medida en una situación.", "Cantidad de residuos, tiempo de llegada o respuesta de encuesta."),
            ("Variable", "Característica que cambia y permite comparar casos.", "Medio de transporte, frecuencia, preferencia o cantidad."),
            ("Muestra", "Parte de una población que se observa con un propósito claro.", "12 estudiantes encuestados de un curso."),
            ("Campo", "Columna que define qué se registra en la tabla.", "Fecha, categoría, cantidad, comentario o fuente."),
            ("Limpieza", "Revisión de datos repetidos, vacíos, mal escritos o sensibles.", "Unificar 'si', 'sí' y 'SI' antes de contar."),
            ("Patrón", "Repetición o tendencia que aparece al ordenar o graficar datos.", "La mayoría llega caminando o en moto."),
            ("Abstracción", "Selección de los datos necesarios sin exponer información innecesaria.", "Usar categorías anónimas en lugar de nombres completos."),
            ("Algoritmo de análisis", "Secuencia de pasos para preguntar, registrar, limpiar, graficar y concluir.", "Pregunta -> tabla -> revisión -> gráfica -> conclusión."),
            ("Phronesis", "Prudencia para decidir con evidencia, contexto, límites y consecuencias.", "No afirmar que 'todos' piensan igual si la muestra fue pequeña."),
            ("Saber comunitario", "Conocimiento construido por observación, memoria y práctica del entorno.", "Una familia decide compras o rutas a partir de experiencia acumulada."),
        ]
    if family == "data_analysis":
        return [
...
def case_text(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "Un curso quiere decidir cómo mejorar el descanso. Algunas familias y estudiantes tienen saberes de experiencia: qué fila se llena primero, en qué momento hay más ruido y qué alimentos se compran más. El equipo recoge datos, pero debe organizarlos sin exponer personas, elegir una gráfica honesta y proponer una decisión que cuide la convivencia."
    if family == "data_analysis":
...
def conceptual_paragraphs(guide: GuideData) -> list[str]:
    ...
    if is_octavo_data_phronesis(guide):
        return [
            f"En {focus}, los datos no reemplazan la experiencia de la comunidad: la complementan. En muchas familias, barrios, veredas y grupos escolares se toman decisiones a partir de observación, memoria, conteo informal y comparación de situaciones repetidas.",
            "Ese conocimiento puede dialogar con herramientas actuales. Una hoja de cálculo ayuda a ordenar registros, pero el estudiante debe preguntar de dónde vienen los datos, qué voces faltan y qué consecuencia tendría la decisión.",
            "El pensamiento computacional organiza el análisis: descomponer la pregunta, reconocer patrones, abstraer solo los campos necesarios, construir un procedimiento y depurar errores antes de concluir.",
            "Una gráfica simple no es decoración. Debe responder la pregunta, mostrar categorías comparables, evitar exageraciones y permitir que otra persona entienda el hallazgo sin que el autor lo explique todo oralmente.",
            "La phronesis invita a decidir con prudencia: afirmar solo lo que la evidencia permite, reconocer límites de la muestra, proteger datos personales y proponer acciones que cuiden la convivencia y el bien común.",
        ]
    if family == "data_analysis":
        return [
...
def worked_examples(guide: GuideData) -> list[tuple[str, str]]:
    ...
    if is_octavo_data_phronesis(guide):
        return [
            ("Ejemplo 1: saber del entorno", "La comunidad sabe que el descanso se congestiona cerca de la tienda. Para estudiarlo, el equipo formula una pregunta: ¿en qué momento hay más fila y qué podría mejorar la convivencia?"),
            ("Ejemplo 2: procedimiento computacional", "Se separa el problema en pasos: definir variable, registrar 10 observaciones, limpiar categorías, contar frecuencias, graficar barras y escribir una conclusión con límite."),
            ("Ejemplo 3: abstracción responsable", "No se necesitan nombres de estudiantes para estudiar una fila. Basta registrar momento, categoría o cantidad, protegiendo la identidad de las personas."),
            ("Ejemplo 4: conclusión prudente", "Si la muestra fue de un solo día, se escribe: 'En esta observación inicial, la fila fue mayor al inicio del descanso; conviene repetir la medición antes de cambiar una norma'."),
        ]
    if family == "data_analysis":
        return [
...
def common_error(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "Error común: creer que una tabla borra la experiencia de la comunidad o que una experiencia personal basta para decidir por todos. Corrección: escuchar saberes del entorno, convertirlos en una pregunta verificable, organizar datos mínimos, revisar errores y concluir con prudencia."
    if family == "data_analysis":
...
def guided_practice_items(guide: GuideData) -> list[str]:
    ...
    if is_octavo_data_phronesis(guide):
        return [
            "Escoge una situación cercana donde exista un saber de experiencia: fila, cuidado de recursos, hábitos de estudio, transporte o convivencia.",
            "Formula una pregunta que pueda responderse con datos y que no exponga información personal.",
            "Descompón el análisis: población o grupo, muestra, variable, campos, fuente y forma de registro.",
            "Registra o simula entre 8 y 12 datos anónimos con categorías comparables.",
            "Limpia la tabla: unifica palabras repetidas, completa vacíos justificados y elimina datos que no necesitas.",
            "Elabora una gráfica simple de barras o un conteo que responda la pregunta.",
            "Escribe una conclusión prudente con evidencia, límite y una recomendación que cuide al grupo.",
        ]
    if family == "data_analysis":
        return [
...
def product_criteria(guide: GuideData) -> list[str]:
    ...
    if is_octavo_data_phronesis(guide):
        return [
            "Pregunta de análisis conectada con un saber familiar, escolar, barrial o comunitario",
            "Tabla con campos, registros y datos anónimos o simulados de forma responsable",
            "Aplicación visible de pensamiento computacional: descomposición, patrón, abstracción y procedimiento",
            "Gráfica simple de barras o conteo que responda la pregunta sin exagerar resultados",
            "Conclusión prudente con evidencia, límite de alcance y recomendación para el bien común",
            "Reflexión socioemocional y ciudadana sobre escucha, respeto, datos y convivencia",
        ]
    if family == "data_analysis":
        return [
...
def learning_goal_text(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "Al terminar la sesión podrás aprender de manera autónoma a transformar una observación del entorno en una pregunta de datos, organizar una tabla sencilla, aplicar pensamiento computacional, construir una gráfica y concluir con prudencia ciudadana."
    if is_noveno_editorial_i(guide):
...
def how_to_use_text(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "Trabaja como investigador de datos del entorno: primero escucha y observa, luego convierte esa experiencia en una pregunta, registra datos mínimos, limpia la tabla, grafica con honestidad y escribe una recomendación que pueda defenderse."
    if is_noveno_editorial_i(guide):
...
def escuta_challenge_text(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "Recuerda una forma en que tu familia, barrio, vereda, grupo deportivo o comunidad escolar toma decisiones por observación y experiencia. Luego piensa cómo podrías convertir esa memoria en una pregunta de datos sin perder respeto por las personas."
    if is_noveno_editorial_i(guide):
...
def escuta_check_items(guide: GuideData) -> list[str]:
    ...
    if is_octavo_data_phronesis(guide):
        return [
            "Reconozco un saber de experiencia que orienta una decisión cercana.",
            "Identifico qué dato actual podría ayudar a comprobar o mejorar esa decisión.",
            "Formulo una pregunta respetuosa, anónima y posible de responder con datos.",
        ]
    if is_noveno_editorial_i(guide):
        return [
...
def escuta_response_prompts(guide: GuideData) -> tuple[str, str]:
    ...
    if is_octavo_data_phronesis(guide):
        return (
            "Un saber familiar, escolar o comunitario que orienta decisiones:",
            "Una pregunta de datos para revisar esa situación hoy:",
        )
    if is_noveno_editorial_i(guide):
        return (
...
def period_idea_text(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "El periodo inicia con análisis de datos en Excel: preguntas, variables, tablas, fórmulas, gráficos y decisiones prudentes, integrando memoria del entorno y pensamiento computacional."
    if family == "systems_room":
...
def praxis_task_text(guide: GuideData) -> str:
    ...
    if is_octavo_data_phronesis(guide):
        return "Elabora un mini análisis de datos sobre una situación cercana. Debe partir de un saber de experiencia, formular una pregunta, organizar una tabla anónima, representar resultados con barras o conteo y escribir una decisión prudente para mejorar la convivencia o el cuidado común."
    if family == "systems_room":
...
Meta personal & Convertir una observación del entorno en una evidencia de datos que otra persona pueda leer y revisar.\\
Recursos & Memoria de la comunidad, tabla de conceptos, datos anónimos o simulados, gráfica simple y lista de criterios.\\
Evidencia mínima & Pregunta, tabla limpia, gráfica de barras o conteo, conclusión prudente y reflexión ciudadana.\\
Control de avance & No concluyo hasta revisar muestra, categorías, datos sensibles, gráfica y límite de mi afirmación.\\
\end{tabularx}
\normalsize
\end{drawbox}
"""
...
def tex_extra_after_escuta(guide: GuideData) -> str:
    if is_octavo_data_phronesis(guide):
        return r"""
\begin{drawbox}{milcVerde}{Puente de saberes: memoria, datos y prudencia}
Elige un saber cercano: una forma de decidir rutas, compras, horarios, turnos, cuidado del agua, uso de espacios o convivencia. No lo trates como superstición ni como verdad automática. Obsérvalo con respeto y pregúntate cómo dialoga con datos actuales.
\begin{enumerate}
\item ¿Qué decisión ayuda a tomar ese saber?
\item ¿Qué observaciones lo sostienen?
\item ¿Qué dato podrías recoger hoy para revisarlo sin exponer a nadie?
\item ¿Qué voz o experiencia no debería quedar por fuera?
\end{enumerate}
\textbf{Dos observaciones que debo respetar:}
\respuesta{2}
\textbf{Un dato que puedo recoger con cuidado:}
\respuesta{1}
\end{drawbox}
"""
    if is_noveno_editorial_i(guide):
...
def tex_extra_after_practice(guide: GuideData) -> str:
    if is_octavo_data_phronesis(guide):
        return r"""
\begin{drawbox}{milcTurquesa}{Pensamiento computacional para entender}
\small
\begin{tabularx}{\textwidth}{>{\bfseries\RaggedRight\arraybackslash}p{3.2cm}YY}
\rowcolor{milcTurquesa!90}\color{white}\bfseries Habilidad & \color{white}\bfseries Pregunta que me hago & \color{white}\bfseries Evidencia en mi guía\\
Descomposición & ¿Qué partes tiene mi problema? & Pregunta, muestra, variable, campos y fuente separados.\\
\rowcolor{milcGris}Patrones & ¿Qué se repite o cambia? & Conteo o gráfica que permite comparar categorías.\\
Abstracción & ¿Qué datos necesito y cuáles debo proteger? & Tabla sin nombres, datos sensibles ni detalles innecesarios.\\
\rowcolor{milcGris}Algoritmo & ¿Qué pasos sigo para que otro repita el análisis? & Procedimiento: recoger, limpiar, contar, graficar y concluir.\\
Depuración & ¿Qué error puede dañar mi conclusión? & Categorías corregidas, vacíos revisados y límite escrito.\\
\end{tabularx}
\normalsize
\end{drawbox}

\begin{softbox}{milcMostaza}{milcGris}{Auto chequeo antes de la praxis}
\checkbox Mi pregunta se puede responder con datos observables.\\
\checkbox Mi tabla protege la identidad de las personas.\\
\checkbox Mi gráfica no exagera ni oculta información importante.\\
\checkbox Mi conclusión dice qué puede afirmar y qué no puede afirmar.\\
\checkbox Mi recomendación cuida convivencia, respeto o bien común.\\
\end{softbox}
"""
    if is_noveno_editorial_i(guide):
        return r"""
...
def tex_extra_after_phase2_bridge(guide: GuideData) -> str:
    if is_octavo_data_phronesis(guide):
        return r"""
\begin{drawbox}{milcTurquesa}{Bitácora de análisis}
Antes de actuar, deja claro tu camino. Esta bitácora evita que la gráfica aparezca sin sentido o que la conclusión se escriba antes de revisar los datos.
\begin{enumerate}
\item Saber o experiencia de partida:
\item Pregunta de datos:
\item Variable principal:
\item Campos de la tabla:
\item Posible error o sesgo que debo vigilar:
\item Límite que debo mencionar al concluir:
\end{enumerate}
\textbf{Mi procedimiento en una frase:}
\respuesta{2}
\end{drawbox}
"""
    if is_undecimo_process_inventory(guide):
...
def tex_extra_after_praxis(guide: GuideData) -> str:
    if is_octavo_data_phronesis(guide):
        return r"""
\begin{drawbox}{milcMagenta}{Del saber al procedimiento}
\small
\begin{tabularx}{\textwidth}{>{\bfseries\RaggedRight\arraybackslash}p{2.4cm}YY}
\rowcolor{milcMagenta!85}\color{white}\bfseries Paso & \color{white}\bfseries Qué hago & \color{white}\bfseries Mi evidencia\\
1. Escuchar & Registro el saber o experiencia sin ridiculizarlo ni idealizarlo. & \\[5mm]
\rowcolor{milcGris}2. Preguntar & Convierto esa experiencia en una pregunta de datos. & \\[5mm]
3. Modelar & Defino variable, muestra, campos y categorías. & \\[5mm]
\rowcolor{milcGris}4. Depurar & Corrijo vacíos, repetidos o datos que no debo usar. & \\[5mm]
5. Visualizar & Construyo barras o conteo que respondan la pregunta. & \\[5mm]
\rowcolor{milcGris}6. Decidir & Escribo conclusión, límite y recomendación ciudadana. & \\[5mm]
\end{tabularx}
\normalsize
\end{drawbox}
"""
...
def tex_extra_after_product(guide: GuideData) -> str:
    if is_octavo_data_phronesis(guide):
        return r"""
\begin{softbox}{milcVino}{milcGris}{Cuidado de datos y voces}
Mi análisis no debe borrar la memoria de las personas ni exponerlas. Por eso uso datos anónimos o simulados, nombro límites de la muestra, respeto opiniones distintas y no presento la gráfica como verdad absoluta.
\end{softbox}
"""
    if is_noveno_editorial_i(guide):
        return r"""
...
def tex_extra_after_evaluation(guide: GuideData) -> str:
    if is_octavo_data_phronesis(guide):
        return r"""
\begin{drawbox}{milcVino}{Reflexión socioemocional y ciudadana}
\checkbox Escuché un saber o experiencia sin burlarme ni imponer mi opinión.\\
\checkbox Reconocí que los datos pueden apoyar decisiones, pero no reemplazan la dignidad de las personas.\\
\checkbox Protegí información personal y evité señalar a compañeros o familias.\\
\checkbox Acepté límites de mi muestra antes de recomendar una acción.\\
\checkbox Mi decisión busca mejorar convivencia, cuidado del espacio o bien común.\\

\textbf{La emoción o actitud que más necesité para trabajar con datos fue:}
\respuesta{1}
\textbf{Una decisión ciudadana que podría mejorar con más evidencia es:}
\respuesta{1}
\end{drawbox}
"""
    return ""
...
def tex_template(guide: GuideData) -> str:
    ...
    today_response_lines = 2
    commitment_response_lines = 2
    symbol_box = r"""
...
"""
    if is_octavo_data_phronesis(guide):
        praxis_box_title = "Mini análisis de datos"
        praxis_title_label = "Título de mi análisis:"
        praxis_explanation_label = "Conclusión prudente:"
        evaluation_sense = "Evaluar es revisar si escuché el contexto, organicé datos con pensamiento computacional, reconocí límites y propuse una decisión respetuosa para la comunidad."
        today_response_lines = 1
        commitment_response_lines = 1
        symbol_box = ""
    if is_noveno_editorial_i(guide):
    ...
    extra_evaluation = tex_extra_after_evaluation(guide)
    evaluation = "\n".join(
...
\respuesta{{{today_response_lines}}}
\textbf{{Mi compromiso para la próxima clase es:}}
\respuesta{{{commitment_response_lines}}}
{symbol_box}
\end{{document}}
"""
...
def write_docx(guide: GuideData, path: Path) -> None:
    ...
    if is_octavo_data_phronesis(guide):
        add_docx_box(
            ...
            "Meta: convertir una observación del entorno en evidencia de datos revisable. Evidencia: pregunta, tabla limpia, gráfica simple, conclusión prudente y reflexión ciudadana.",
            COLORS["verde"],
        )
    if is_noveno_editorial_i(guide):
        add_docx_box(
    ...
    for label, color, body in [
        ...
        if "Escucha" in label:
            ...
            if is_undecimo_process_inventory(guide):
                ...
                add_docx_lines(doc, 3)
            if is_octavo_data_phronesis(guide):
                doc.add_paragraph("Puente de saberes: memoria, datos y prudencia").runs[0].bold = True
                for item in [
                    "Registra una decisión que tu familia, barrio, vereda o grupo escolar suele tomar por experiencia.",
                    "Pregunta qué dato actual permitiría revisar esa decisión sin exponer a ninguna persona.",
                    "Escribe qué voz o experiencia no debería quedar por fuera del análisis.",
                ]:
                    doc.add_paragraph(item, style=None)
                add_docx_lines(doc, 3)
            ...
            if is_undecimo_process_inventory(guide):
                ...
                for item in [
                    ...
                    doc.add_paragraph(f"☐ {item}")
            if is_octavo_data_phronesis(guide):
                doc.add_paragraph("Pensamiento computacional para entender:").runs[0].bold = True
                table = doc.add_table(rows=1, cols=3)
                table.style = "Table Grid"
                for i, h in enumerate(["Habilidad", "Pregunta que me hago", "Evidencia"]):
                    shade_cell(table.cell(0, i), COLORS["turquesa"])
                    set_cell_text(table.cell(0, i), h, True, "FFFFFF")
                for row in [
                    ("Descomposición", "¿Qué partes tiene mi problema?", "Pregunta, muestra, variable, campos y fuente separados."),
                    ("Patrones", "¿Qué se repite o cambia?", "Conteo o gráfica que compara categorías."),
                    ("Abstracción", "¿Qué datos necesito y cuáles debo proteger?", "Tabla sin nombres ni datos sensibles."),
                    ("Algoritmo", "¿Qué pasos sigo para que otro repita el análisis?", "Recoger, limpiar, contar, graficar y concluir."),
                    ("Depuración", "¿Qué error puede dañar mi conclusión?", "Categorías corregidas, vacíos revisados y límite escrito."),
                ]:
                    cells = table.add_row().cells
                    for i, value in enumerate(row):
                ...
                doc.add_paragraph("Auto chequeo antes de la praxis:").runs[0].bold = True
                for item in [
                    "Mi pregunta se puede responder con datos observables.",
                    "Mi tabla protege la identidad de las personas.",
                    "Mi gráfica no exagera ni oculta información importante.",
                    "Mi conclusión dice qué puede afirmar y qué no puede afirmar.",
                    "Mi recomendación cuida convivencia, respeto o bien común.",
                ]:
                    doc.add_paragraph(f"☐ {item}")
            ...
            if is_octavo_data_phronesis(guide):
                add_docx_box(
                    ...
                    "Antes de actuar, define saber de partida, pregunta, variable, campos, posible sesgo y límite de la conclusión.",
                    COLORS["turquesa"],
                )
                for item in [
                    "Saber o experiencia de partida.",
                    "Pregunta de datos.",
                    "Variable principal.",
                    "Campos de la tabla.",
                    "Posible error o sesgo que debo vigilar.",
                    "Límite que debo mencionar al concluir.",
                ]:
            ...
            if is_undecimo_process_inventory(guide):
                ...
                for item in ["Proceso elegido", "Impacto", "Facilidad", "Riesgo y datos", "Oportunidad futura"]:
                    ...
                    add_docx_lines(doc, 1)
            if is_octavo_data_phronesis(guide):
                doc.add_paragraph("Del saber al procedimiento:").runs[0].bold = True
                for item in [
                    "Escuchar: registro el saber o experiencia con respeto.",
                    "Preguntar: convierto esa experiencia en una pregunta de datos.",
                    "Modelar: defino variable, muestra, campos y categorías.",
                    "Depurar: corrijo vacíos, repetidos o datos que no debo usar.",
                    "Visualizar: construyo barras o conteo que respondan la pregunta.",
                    "Decidir: escribo conclusión, límite y recomendación ciudadana.",
                ]:
                    doc.add_paragraph(item)
                    add_docx_lines(doc, 1)
        else:
            ...
            if is_octavo_data_phronesis(guide):
                add_docx_box(
                    doc,
                    "Cuidado de datos y voces",
                    "Usa datos anónimos o simulados, nombra límites de la muestra, respeta opiniones distintas y evita presentar la gráfica como verdad absoluta.",
                    COLORS["vino"],
                )
            if is_undecimo_process_inventory(guide):
                add_docx_box(
            ...
            for row in evaluation_rows(guide):
                ...
                for i, value in enumerate(row):
                    set_cell_text(cells[i], value, bold=(i == 0))
            if is_octavo_data_phronesis(guide):
                doc.add_paragraph("Reflexión socioemocional y ciudadana:").runs[0].bold = True
                for item in [
                    "Escuché un saber o experiencia sin burlarme ni imponer mi opinión.",
                    "Reconocí que los datos apoyan decisiones, pero no reemplazan la dignidad de las personas.",
                    "Protegí información personal y evité señalar a compañeros o familias.",
                    "Acepté límites de mi muestra antes de recomendar una acción.",
                    "Mi decisión busca mejorar convivencia, cuidado del espacio o bien común.",
                ]:
                    doc.add_paragraph(f"☐ {item}")
                doc.add_paragraph("La emoción o actitud que más necesité para trabajar con datos fue:")
                add_docx_lines(doc, 1)
                doc.add_paragraph("Una decisión ciudadana que podría mejorar con más evidencia es:")
                add_docx_lines(doc, 1)
            doc.add_paragraph("Hoy entendí que:")
            add_docx_lines(doc, 1 if is_octavo_data_phronesis(guide) else 2)
            doc.add_paragraph("Mi compromiso:")
            add_docx_lines(doc, 1 if is_octavo_data_phronesis(guide) else 2)
            if not is_octavo_data_phronesis(guide):

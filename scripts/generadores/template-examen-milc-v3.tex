...
% ── Portada ───────────────────────────────────────────────────────────
% Antes: pagina entera en negro. Es el documento que mas se imprime del
% catalogo —uno por estudiante— y esa plancha vaciaba el toner del colegio.
% La banda ahora ocupa una cuarta parte; el resto va en blanco.
\begin{tikzpicture}[remember picture,overlay]
  \path[fill=examenFondo]
    (current page.north west) rectangle ([yshift=-6.8cm]current page.north east);

  \node[anchor=north west,text=examenTexto] at ([xshift=2.0cm,yshift=-1.75cm]current page.north west)
    {\sffamily\bfseries\fontsize{13}{15}\selectfont EXAMEN FINAL};
  \node[anchor=north west,text=examenTextoSubtle] at ([xshift=2.0cm,yshift=-2.6cm]current page.north west)
    {\sffamily\bfseries\fontsize{9}{11}\selectfont GRADO <<<GRADO>>>° · PERÍODO <<<PERIODO>>> · TECNOLOGÍA E INFORMÁTICA};

  \node[fill=milcPeriodoPrimary,text=white,rounded corners=2mm,
        inner xsep=4mm,inner ysep=2mm,anchor=north east]
    at ([xshift=-2.0cm,yshift=-1.75cm]current page.north east)
    {\sffamily\bfseries\fontsize{9}{11}\selectfont APLICACIÓN · SESIÓN 10};

  \node[anchor=west,text=examenTexto,text width=17cm,align=left] at ([xshift=2.0cm,yshift=-5.0cm]current page.north west)
    {\sffamily\bfseries\fontsize{26}{30}\selectfont <<<TITULO_PORTADA>>>};
\end{tikzpicture}

\vspace*{5.6cm}

{\sffamily\fontsize{14}{19}\selectfont Síntesis · aplicación · argumentación.\\
...
{\sffamily\bfseries\fontsize{15}{18}\selectfont <<<DURACION_MIN>>> min}
\end{tcolorbox}
&
\begin{tcolorbox}[enhanced,colback=white,colframe=milcLinea,arc=2mm,boxrule=1pt,
  left=4mm,right=4mm,top=3mm,bottom=3mm,nobeforeafter]
{\sffamily\bfseries\fontsize{8}{10}\selectfont\textcolor{milcNegro!55}{APLICACIÓN}}\\[1.5mm]
{\sffamily\bfseries\fontsize{15}{18}\selectfont <<<FECHA_APLICACION>>>}
\end{tcolorbox}
&
\begin{tcolorbox}[enhanced,colback=white,colframe=milcLinea,arc=2mm,boxrule=1pt,
  left=4mm,right=4mm,top=3mm,bottom=3mm,nobeforeafter]
{\sffamily\bfseries\fontsize{8}{10}\selectfont\textcolor{milcNegro!55}{PONDERACIÓN}}\\[1.5mm]
{\sffamily\bfseries\fontsize{15}{18}\selectfont <<<PONDERACION>>>\%}
\end{tcolorbox}
\end{tabularx}
...
{\sffamily\bfseries\fontsize{8}{10}\selectfont\textcolor{milcNegro!55}{DATOS DEL ESTUDIANTE}}\\[4mm]
\begin{tabularx}{\linewidth}{@{}p{3.4cm}Y@{}}
Nombre completo: & \color{milcLinea}\rule{\linewidth}{0.6pt}\color{milcNegro}\\[5mm]
Grado y grupo: & \color{milcLinea}\rule{\linewidth}{0.6pt}\color{milcNegro}\\[5mm]
Fecha: & \color{milcLinea}\rule{\linewidth}{0.6pt}\color{milcNegro}\\[5mm]
Firma: & \color{milcLinea}\rule{\linewidth}{0.6pt}\color{milcNegro}\\
\end{tabularx}
\end{tcolorbox}
...
{\sffamily\fontsize{10}{13}\selectfont Institución Educativa Sor María Juliana · Cartago, Valle del Cauca\\
